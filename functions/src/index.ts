    ...
    try {
      ...
      try {
        ...
        try {
          ...
          if (snap.exists) {
            const tokens = snap.data() as Credentials | undefined;
            if (tokens) {
          ...
          try {
            ...
            const kpsLabel = labels.find((label: gmail_v1.Schema$Label) => label.name === 'KPS');
          ...
          try {
            ...
            const labelsResponse = await gmail.users.labels.list({ userId: 'me' });
            const labels = labelsResponse.data.labels ?? [];
            const contactFormLabel = labels.find((label: gmail_v1.Schema$Label) => label.name === 'Contact Form');
          ...
          try {
            const draftMessageId = draft.data.message?.id;
            if (draftMessageId) {
              ...
              console.log('⚠️ Draft message ID missing; cannot move draft to label');
            }
